    ...
    Interceptor.attach(sub_1CFF0, {
        onEnter: function(args) {
            ...
            Stalker.follow(this.tid, {
                ...
                onReceive(events) {
                    console.log('onReceive : ', Stalker.parse(events));
                    for (var [index, value] of Object.entries(events)) {
                        if (value.indexOf('exec') != -1) {
                            console.log('onReceive index : ', index, ', value : ', value);
                            var address = value.toString().split(',');
                            var addr = address[1];
                            var module = Process.findModuleByAddress(addr);
                            if (module) {
                                console.log('onReceive module : ', module.name + '!' + ptr(addr).sub(module.base), Instruction.parse(ptr(addr)));
                            }
                        }
                    }
                },
